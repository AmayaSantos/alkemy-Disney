...
public class MovieService {
    ...
    public Object updateMovie(MovieRequest movieRequest) {
        Movie movie;
        Optional<Movie> optionalMovie=movieRepository.findOneByIdOrTitle(movieRequest.getId(),movieRequest.getTitle());
    ...
    public Movie saveNewMovie(MovieRequest movieRequest) {
        movieRepository.findOneByTitle(movieRequest.getTitle()).orElseThrow(
                ()-> new RuntimeException("Could not store Movie " + movieRequest.getTitle() +"is ready exist"));
    ...
    public Movie addCharacterToMovie(Long movie_id, Long character_id) {
        Movie movie= movieRepository.findById(movie_id).orElseThrow(() -> new RuntimeException("Error: Movie is not found."));
    ...
    public void deleteMovie(Long movie_id) {
        movieRepository.delete(movieRepository.findById(movie_id).orElseThrow(() -> new RuntimeException("Error: Movie is not found.")));
    }
