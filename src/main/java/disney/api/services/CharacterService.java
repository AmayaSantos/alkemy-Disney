...
public class CharacterService {
  ...
  public Character saveNewCharacter(CharacterRequest characterRequest) {
    characterRepository
        .findOneByName(characterRequest.getName())
        .orElseThrow(
            () ->
                new RuntimeException(
                    "Could not store Character " + characterRequest.getName() + "is ready exist"));
  ...
  private Character saveCharacter(Character character, CharacterRequest characterRequest) {
    try {
      ...
      throw new RuntimeException(
          "Could not store Character " + characterRequest.getName() + ". Please try again!", ex);
    }
  }

  private Character storeImgInCharacter(Character character, MultipartFile file)
      throws IOException {
  ...
  public CharacterResponse getByName(String name) {
    return new CharacterResponse(characterRepository.findOneByName(name).orElse(new Character()));
  }

  public List<CharacterResponse> getByAge(Integer age) {
    return castCharactesResponse(characterRepository.findByAge(age));
  }

  public List<CharacterResponse> getByFilm(String title) {
    return castCharactesResponse(characterRepository.findByFilms(title));
  }

  private List<CharacterResponse> castCharactesResponse(List<Character> characters) {
    List<CharacterResponse> characterResponses = new ArrayList<>();
    characters.forEach(
        character -> {
          CharacterResponse characterResponse = new CharacterResponse(character);
